...
class AttachmentService:
    ...
    async def upload_temporary_attachment(
        self, db: Session, file: UploadFile, media_type: MediaType | None = None
    ) -> PromptAttachment:
        """临时上传附件（不关联提示词）
        ...
        (
            ...
            error_msg,
            detected_mime,
        ) = await self.validation_service.validate_upload_file(file, target_media_type)
        if not is_valid:
            raise HTTPException(status_code=400, detail=error_msg)

        try:
            # 保存原始文件
            filename, file_path = await self.storage_service.save_file(
                file, "attachments"
            )
            ...
            db.rollback()
            raise HTTPException(
                status_code=500, detail=f"附件上传失败: {str(e)}"
            ) from e
    ...
    def get_attachment(
        ...
        """
        return (
            db.query(PromptAttachment)
            .filter(PromptAttachment.id == attachment_id)
            .first()
        )

    def get_prompt_attachments(
        self, db: Session, prompt_id: int, skip: int = 0, limit: int = 100
        ...
        return (
            db.query(PromptAttachment)
            .filter(PromptAttachment.prompt_id == prompt_id)
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def update_attachment_metadata(
        self, db: Session, attachment_id: int, metadata: dict[str, Any]
    ) -> PromptAttachment | None:
        """更新附件元数据
    ...
    def validate_attachment_access(
        self, db: Session, attachment_id: int, prompt_id: int | None = None
    ) -> PromptAttachment | None:
        """验证附件访问权限
        ...
        """
        query = db.query(PromptAttachment).filter(PromptAttachment.id == attachment_id)
    ...
    def get_storage_statistics(self, db: Session) -> dict[str, Any]:
        ...
        for media_type in MediaType:
            ...
            count = (
                db.query(PromptAttachment)
                .join(Prompt)
                .filter(Prompt.media_type == media_type)
                .count()
            )
    ...
    def attach_to_prompt(
        self, db: Session, attachment_ids: list[int], prompt_id: int
    ) -> list[PromptAttachment]:
        """将临时附件关联到提示词
        ...
        attachments = (
            db.query(PromptAttachment)
            .filter(PromptAttachment.id.in_(attachment_ids))
            .all()
        )
        ...
        try:
            ...
            db.rollback()
            raise HTTPException(
                status_code=500, detail=f"关联附件失败: {str(e)}"
            ) from e
    ...
    def get_temporary_attachments(
        ...
        return (
            db.query(PromptAttachment)
            .filter(PromptAttachment.prompt_id.is_(None))
            .offset(skip)
            .limit(limit)
            .all()
        )
