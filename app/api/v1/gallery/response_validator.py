            ...
            if "pagination" in response_data:
                ...
                if not isinstance(pagination, dict):
                    ...
                    for field in ["page", "pageSize", "total", "totalPages"]:
                        if field in pagination and not isinstance(
...
def validate_gallery_response_format(response_data: Any) -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: 验证结果
    """
    result = {"valid": False, "errors": [], "warnings": [], "response_type": None}
...
def test_response_formats():
    ...
    test_cases = [
        ...
        # 无效响应 - 缺少success字段
        {"name": "无效响应 - 缺少success字段", "data": {"data": [{"id": 1}]}},
        # 无效响应 - success类型错误
        {"name": "无效响应 - success类型错误", "data": {"success": "true", "data": []}},
    ]
    ...
    for test_case in test_cases:
        print(f"\n测试用例: {test_case['name']}")
        print(
            f"输入数据: {json.dumps(test_case['data'], ensure_ascii=False, indent=2)}"
        )
