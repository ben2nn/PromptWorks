...
class GalleryNotFoundError(GalleryException):
    ...
    def __init__(self, resource: str = "资源"):
        super().__init__(
            code="RESOURCE_NOT_FOUND", message=f"{resource}不存在", status_code=404
        )
...
class GalleryResponse:
    ...
    def error(code: str, message: str, details: Any = None) -> Dict[str, Any]:
        """错误响应格式"""
        error_response = {"success": False, "error": {"code": code, "message": message}}
        if details:
    ...
    if isinstance(exc, IntegrityError):
        ...
        return JSONResponse(
            ...
            content=GalleryResponse.error(
                "DATABASE_INTEGRITY_ERROR", "数据完整性约束违反"
            ),
        )
