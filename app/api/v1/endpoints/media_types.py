...
def get_media_type_statistics(db: Session = Depends(get_db)) -> dict[str, Any]:
    """获取媒体类型统计信息"""

    # 查询各媒体类型的提示词数量
    stmt = select(Prompt.media_type, func.count(Prompt.id).label("count")).group_by(
        Prompt.media_type
    )
    ...
    for media_type in MediaType:
        if media_type.value not in statistics:
            statistics[media_type.value] = {"count": 0, "percentage": 0}

    return {
        "total_prompts": total_prompts,
        "by_media_type": statistics,
        "most_used": max(statistics.items(), key=lambda x: x[1]["count"])[0]
        if statistics
        else None,
    }
...
def get_media_type_info(media_type: MediaType) -> dict[str, Any]:
    """获取特定媒体类型的详细信息"""
...
def get_media_type_validation_rules(media_type: MediaType) -> dict[str, Any]:
    """获取媒体类型的验证规则"""
    ...
    return {
        ...
        "max_size_mb": info.get("max_size") // (1024 * 1024)
        if info.get("max_size")
        else None,
    }
