...
async def upload_attachment(
    ...
    if prompt_id == 0:
        attachment = await attachment_service.upload_temporary_attachment(
            db=db, file=file, media_type=media_type
        )
        message = "文件临时上传成功"
    else:
        # 正常上传并关联
        attachment = await attachment_service.upload_attachment(
            db=db, prompt_id=prompt_id, file=file, media_type=media_type
        )
        message = "文件上传成功"
    ...
    return AttachmentUploadResponse(attachment=attachment_read, message=message)
...
def list_prompt_attachments(
    ...
    attachments = attachment_service.get_prompt_attachments(
        db=db, prompt_id=prompt_id, skip=skip, limit=limit
    )
    ...
    # 转换为响应模式
    attachment_reads = [
        attachment_service.to_attachment_read(attachment) for attachment in attachments
    ]

    return AttachmentListResponse(items=attachment_reads, total=total)
...
def get_attachment(attachment_id: int, db: Session = Depends(get_db)) -> AttachmentRead:
    """获取单个附件信息"""
...
def download_attachment(
    ...
    if not file_storage_service.file_exists(attachment.file_path):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="文件不存在或已被删除"
        )
...
def get_attachment_thumbnail(
    ...
    if not attachment.thumbnail_path:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="该附件没有缩略图"
        )
    ...
    if not file_storage_service.file_exists(attachment.thumbnail_path):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="缩略图文件不存在或已被删除"
        )
...
def update_attachment(
    attachment_id: int, payload: AttachmentUpdate, db: Session = Depends(get_db)
) -> AttachmentRead:
    """更新附件信息"""
    ...
    if payload.file_metadata is not None:
        updated_attachment = attachment_service.update_attachment_metadata(
            db=db, attachment_id=attachment_id, metadata=payload.file_metadata
        )
...
def delete_attachment(attachment_id: int, db: Session = Depends(get_db)) -> Response:
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="删除附件失败"
        )
...
def batch_update_attachment_prompt(
    prompt_id: int, attachment_ids: list[int], db: Session = Depends(get_db)
) -> list[AttachmentRead]:
    """批量更新附件的 prompt_id
    ...
    if not attachment_ids:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="附件 ID 列表不能为空"
        )

    # 关联附件
    attachments = attachment_service.attach_to_prompt(
        db=db, attachment_ids=attachment_ids, prompt_id=prompt_id
    )
...
@router.get("/statistics", summary="获取存储统计", description="获取附件存储的统计信息")
def get_storage_statistics(db: Session = Depends(get_db)) -> dict:
    """获取存储统计信息"""
