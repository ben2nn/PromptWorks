...
class TestAttachmentService:
    ...
    async def test_upload_attachment_success(
        self, db_session: Session, sample_prompt: Prompt, sample_image_file: UploadFile
    ):
        """测试成功上传附件"""
        with (
            patch.object(file_storage_service, "save_file") as mock_save_file,
            patch.object(
                attachment_service.thumbnail_service, "is_image_file"
            ) as mock_is_image,
            patch.object(
                attachment_service.thumbnail_service, "process_image"
            ...
            attachment = await attachment_service.upload_attachment(
                db=db_session, prompt_id=sample_prompt.id, file=sample_image_file
            )
            ...
            db_attachment = (
                ...
                .filter(PromptAttachment.id == attachment.id)
                .first()
            )
    ...
    async def test_upload_attachment_prompt_not_found(
        self, db_session: Session, sample_image_file: UploadFile
    ):
        """测试上传附件时提示词不存在"""
    ...
    async def test_upload_attachment_validation_failure(
        ...
        with pytest.raises(HTTPException) as exc_info:
            await attachment_service.upload_attachment(
                db=db_session, prompt_id=sample_prompt.id, file=invalid_file
            )
    ...
    def test_count_prompt_attachments(self, db_session: Session, sample_prompt: Prompt):
        ...
        # 统计数量
        count = attachment_service.count_prompt_attachments(
    ...
    def test_delete_prompt_attachments(
        self, db_session: Session, sample_prompt: Prompt
    ):
        """测试删除提示词的所有附件"""
        # 创建多个测试附件
    ...
    def test_get_attachment_download_url(self, sample_prompt: Prompt):
        ...
        with patch.object(file_storage_service, "get_file_url") as mock_get_url:
            mock_get_url.return_value = (
                "http://localhost/api/v1/files/attachments/test_file.png"
            )
    ...
    def test_get_attachment_thumbnail_url(self, sample_prompt: Prompt):
        ...
        with patch.object(file_storage_service, "get_file_url") as mock_get_url:
            mock_get_url.return_value = (
                "http://localhost/api/v1/files/thumbnails/test_thumb.jpg"
            )
    ...
    def test_to_attachment_read(self, sample_prompt: Prompt):
        ...
        with (
            ...
            patch.object(
                attachment_service, "get_attachment_thumbnail_url"
            ) as mock_thumb_url,
        ):
    ...
    def test_validate_attachment_access(
        self, db_session: Session, sample_prompt: Prompt
    ):
        """测试验证附件访问权限"""
        # 创建测试附件
    ...
    def test_get_storage_statistics(self, db_session: Session, sample_prompt: Prompt):
        ...
        # 创建文档分类
        doc_class = PromptClass(name="文档分类", description="文档类型分类")
        ...
        with patch.object(
            file_storage_service, "get_storage_info"
...
class TestFileValidationService:
    ...
    async def test_validate_upload_file_success(self):
        ...
        upload_file = UploadFile(
            filename="test.png", file=file_obj, size=len(png_content)
        )
